...
NS_Ag::RandomKey::RandomKey(Instancia &instancia)
{
    ...
    vetRandom = Vector<RandomNo>(tam);
    vetDecod  = Vector<RandomNo>(tam);
...
bool NS_Ag::atendeTodosOsClientes(const Vector<int> &vetClienteAtend)
{
...
void NS_Ag::decodificaSol(Instancia &instancia, RandomKey &randKey, Satelite &sat, const Vector<int> &vetSatAtendCliente, const double tempoSaidaSat)
{
    ...
    static Vector<bool> vetBackTrack(instancia.numNos*2);        // Indica se a posicao deve ser testada novamente, no mesmo veic
